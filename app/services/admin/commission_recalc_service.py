...
from app.services.commission_service import process_commission
...
def recalc_user_commissions(user_id, admin_id):
    ...
    try:
        ...
        # log admin action
        cursor.execute("""
            INSERT INTO commission_recalc_logs
            (admin_id, target_user_id, recalc_type)
            VALUES (%s, %s, %s)
        """, (
            admin_id,
            user_id,
        ...
        return {
            "total_events": total_recalculated
        }
...
def recalc_purchase_commission(purchase_id, admin_id):
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO commission_recalc_logs
            (admin_id, reference_id, recalc_type)
            VALUES (%s, %s, %s)
        """, (
            admin_id,
            purchase_id,
...
def recalc_date_commissions(date, admin_id):
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO commission_recalc_logs
            (admin_id, recalc_type, remark)
            VALUES (%s, %s, %s)
        """, (
            admin_id,
            "date_recalc",
            f"Recalculated commissions for {date}"
        ))
