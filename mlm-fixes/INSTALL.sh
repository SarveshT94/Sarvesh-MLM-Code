...
FILES=(
  ...
  "app/services/admin/tree_service.py" "app/services/report_service.py"
  "app/services/withdraw_service.py" "app/services/user_service.py"
  "app/services/store_service.py"
  "app/routes/admin/package_routes.py" "app/routes/team_routes.py"
  "app/routes/store_routes.py" "app/routes/admin/store_routes.py"
  "app/templates/admin/user_team.html" "app/templates/admin/packages.html"
  "app/templates/admin/financial_report.html"
  "app/templates/admin/store_dashboard.html" "app/templates/admin/store_products.html"
  "app/templates/admin/store_product_edit.html" "app/templates/admin/store_orders.html"
  "app/templates/admin/store_order_detail.html"
  ...
  "frontend/src/components/store/Storefront.jsx"
  "frontend/src/components/store/MyStoreOrders.jsx"
)
...
# ---------------------------------------------------------------------------
c_b "[3/7] Patching app/routes/main.py + admin menu ..."
python3 "$FIXES_DIR/deploy/patch_main.py" "$TARGET/app/routes/main.py" || \
  c_r "   ! main.py patch failed (tell support)"
python3 "$FIXES_DIR/deploy/patch_admin_nav.py" "$TARGET" || \
  c_y "   ! admin menu patch skipped (Store pages still reachable at /admin/store)"
mkdir -p "$TARGET/app/static/uploads/products"
# sample product photos shipped with the fixes (admin can replace them)
if [[ -d "$FIXES_DIR/app/static/uploads/products" ]]; then
  cp -n "$FIXES_DIR"/app/static/uploads/products/* "$TARGET/app/static/uploads/products/" 2>/dev/null || true
  echo  "   ✓ sample product images installed"
fi

# ---------------------------------------------------------------------------
# 4. Wire the Next.js dashboard
# ---------------------------------------------------------------------------
c_b "[4/7] Wiring the Next.js dashboard (My Team widget + Shop + My Orders) ..."
...
if command -v psql >/dev/null 2>&1 && psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -tAc "SELECT 1" >/dev/null 2>&1; then
  ...
  if [[ "${DB_OK:-1}" -eq 1 ]]; then
    ...
    if [[ $MIG_FAIL -eq 0 ]]; then
      ...
      psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -tAc \
        "SELECT '     store categories: ' || COUNT(*) FROM product_categories;" 2>/dev/null
      psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -tAc \
        "SELECT '     store products: ' || COUNT(*) || ' (' || (SELECT COUNT(*) FROM product_variants) || ' variants)' FROM products;" 2>/dev/null
    fi
  fi
else
...
echo  "     • Admin → Store Dashboard : shipping fee, plan-match rule, categories"
echo  "     • Admin → Products & Categories : add products, variants, images, stock"
echo  "     • Admin → Store Orders & Tracking : confirm → packed → shipped → delivered"
echo  "     • Member dashboard → Shop : build a cart to a plan amount and pay"
echo  "     • Member dashboard → My Orders : tracking + GST invoice"
echo  "     • Admin → Reports : P&L now includes store COGS, GST and repurchases"
echo
c_b "  ONLINE PAYMENTS (Razorpay) — add to .env when you have the keys:"
echo  "     RAZORPAY_KEY_ID=rzp_live_xxxx"
echo  "     RAZORPAY_KEY_SECRET=xxxx"
echo  "     RAZORPAY_WEBHOOK_SECRET=xxxx   (Razorpay dashboard → Webhooks → /webhook/payment)"
echo  "     Until then, members can pay from wallet, and admin can 'Mark payment received'."
c_g "=================================================================="
