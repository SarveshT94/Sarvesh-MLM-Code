...
def authenticate_user(identifier, raw_password):
    try:
        ...
        # NOTE (E-commerce + MLM): a member registers FIRST and activates by
        # SHOPPING (cart = plan amount). So a not-yet-activated member
        # (is_active = FALSE) MUST be able to log in and use the store.
        # Their dashboard shows an "Activate" state until the first purchase.
        ...
        return {"status": "success", "message": "Login successful.",
                "user": {**user, "is_active": bool(user.get('is_active', False))}}
